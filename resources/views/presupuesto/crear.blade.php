...
@section('container')
    ...
    <form action="{{route('presupuestos.crear')}}" method="post">
    {{csrf_field()}}
    <div class="row">
        <div class="col-lg-4">
            <div class="form-group">
                <label for="vencimiento">Fecha Vencimiento:</label>
                <input name="vencimiento" type="date" class="form-control" id="vencimiento" value="{{ old('vencimiento') }}" aria-describedby="emailHelp" placeholder="Ingresar la descripción...">
                @error('vencimiento')
    ...
    <div class="row">
            ...
            <div class="form-group">
                ...
                <select name="producto_id" id="producto_id" class="form-control selectpicker" data-live-search="true" data-size="3" data-dropup-auto="false">
                    ...
                    @foreach (getProducto() as $producto)
                        <option value="{{$producto->id}}_{{ $producto->precio_unitario }}">{{$producto->nombre}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="col-lg-2">
            <div class="form-group">
                ...
                <input type="number" disabled name="precio" id="precio" class="form-control " placeholder="Precio....">
            </div>
        </div>
    ...
    <div class="row">
        ...
        <div class="col-lg-1 col-sm-6 col-md-6 col-xl-12" id="guardar">
            <div class="form-group">
                ...
                @csrf
                <button type="submit" class="btn btn-success"><i class="fas fa-save"></i> Registrar Presupuesto</button>
            </div>
        </div>
        <div class="col-lg-1 col-sm-6 col-md-6 col-xl-12">
            <div class="form-group">
                <a href="{{ route('presupuestos.index') }}"><button type="button" class="btn btn-primary" ><i class="fas fa-arrow-left"></i> Volver</button></a>
            </div>
        </div>
    </form>
    @push('scripts')
        ...
        function agregar(){
            ...
            if (producto_id != "" && cantidad != "" && precio !="") {
                if (cantidad > 0 && precio >= 0) {
                    ...
                    var fila = '<tr class="selected" id="fila'+cont+'"><td><button type="button" class="btn btn-warning" onclick="eliminar('+cont+');">X</button></td><td><input type="hidden" name="producto_id[]" value="'+producto_id+'">'+producto+'</td><td><input type="text" name="precio[]" readonly value="'+precio+'"> </td><td><input type="text" name="cantidad[]" readonly value="'+cantidad+'"></td> <td>'+(subtotal[cont].toFixed(2))+'</td></tr>';
                    cont++;
                    limpiar();
                    totales();
                    evaluar();
        ...
        function totales(){
            $("#total-test").html(total.toFixed(2) + "Bs.");
            $("#total").val(total);
        }
